...
// native identity arrives once Amp reports it.
...
func (a *Agent) NewSession(ctx context.Context, params acp.NewSessionRequest) (resp acp.NewSessionResponse, err error) {
	...
	if err := s.createNative(ctx); err != nil {
		return acp.NewSessionResponse{}, err
	}
...
func (a *Agent) restore(
	...
	if active != nil {
		...
		if sameCarrier(active, start) {
			return a.restoreActive(ctx, active, start, replay, release)
		}
	...
	s := a.restoreCandidate(start, sessionID, stored)

	rows, err := s.hydrate(ctx, stored)
	if err != nil {
		return nil, nil, err
	}
...
func (a *Agent) restoreCandidate(start sessionStart, sessionID acp.SessionId, stored storedSession) *session {
	...
	s.serviceURL = stored.record.ServiceURL
	s.usage = cloneUsage(stored.record.Usage)
...
// restoreActive publishes a verified candidate before changing the live native binding.
func (a *Agent) restoreActive(ctx context.Context, s *session, start sessionStart, replay bool, release func()) (*session, func(), error) {
	...
	// A failed earlier commit can leave a newer verified generation in memory.
	stored.record = s.record()
	stored.rows = s.rows
	candidate := a.restoreCandidate(start, s.id, stored)

	rows, err := candidate.hydrate(ctx, stored)
	if err != nil {
		release()
	...
	candidate.rows = rows
	if err := candidate.commitMirror(ctx); err != nil {
		candidate.discardUnbound(ctx, stored.record)
		release()

		return nil, nil, a.restoreRefused(ctx, s.id, err)
	}

	if candidate.nativeID != s.nativeID {
		s.fenceStream()
	}

	s.mu.Lock()
	s.nativeID = candidate.nativeID
	s.serviceURL = candidate.serviceURL
	s.rows = rows
	s.usage = candidate.usage
	s.rawEvents = candidate.rawEvents
	s.mu.Unlock()
...
func inheritCarrier(meta sessionMeta, record sessionRecord) AmpOptions {
...
func (a *Agent) Prompt(ctx context.Context, params acp.PromptRequest) (resp acp.PromptResponse, err error) {
	if a.restores.Pending(params.SessionId) {
...
// session with the same id. The native thread remains on Amp's service.
