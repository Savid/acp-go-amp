...
// The generated-native ownership handoff re-reads every descriptor it is about
// to trust, so each syscall it depends on is reached through a seam. Faulting a
// seam is the only way to prove the handoff fails closed when the kernel stops
...
var (
	...
	generatedNativeReadDir = func(directory *os.File) ([]os.DirEntry, error) {
...
// generatedNativeSharedIdentityAncestryRemedy states what an operator can
// change when the trusted identity is also the target identity and an ancestor
// still fails the walk. There is no privilege boundary left to lean on in that
// shape, so the remaining answers are to give the supervisor one, or to anchor
// the tree on a path the identity already owns.
const generatedNativeSharedIdentityAncestryRemedy = "run the supervisor as root to isolate the agent identity, " +
	"or place the native directory under a path the agent identity owns"

// generatedNativeAncestorIsSharedIdentityRoot reports whether a non-final
// ancestor is acceptable purely because root owns it. It is, only when the
// trusted identity and the target identity are the same one: owning both ends of
// the handoff leaves no privilege boundary for the ancestry rule to defend,
// while every path to a directory that identity owns still crosses root-owned
// components such as "/" and "/home". An ancestor owned by any other identity
// stays refused, so a second local identity still cannot interpose one.
func generatedNativeAncestorIsSharedIdentityRoot(stat unix.Stat_t, final bool, trustedUID uint32, targetUID uint32) bool {
...
func validateGeneratedNativeAncestor(
	...
	rootOwned := generatedNativeAncestorIsSharedIdentityRoot(stat, final, trustedUID, targetUID)
	if !trusted && !rootOwned {
		if !final && trustedUID == targetUID {
			return fmt.Errorf(
				"generated native path ancestor is uid=%d gid=%d; %s",
				stat.Uid, stat.Gid, generatedNativeSharedIdentityAncestryRemedy,
			)
		}
