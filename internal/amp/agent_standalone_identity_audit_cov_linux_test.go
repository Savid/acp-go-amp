...
func agentStandaloneCovWriteActiveMarker(t *testing.T, directory *os.File, owner agentStandaloneOwner) {
	t.Helper()
	key := agentStandaloneOwnerDigest(owner)
	agentStandaloneCovWriteRegistryFile(
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62609, 62610, "no-domain", "/srv/amp/no-domain", 1, 2),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovPermanentLock(t, directory, "owners.lock")
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62621, 62622, "lockless", "/srv/amp/lockless", 1, 2),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62623, 62624, "gid-a", "/srv/amp/gid-a", 1, 2),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62625, 62624, "gid-b", "/srv/amp/gid-b", 3, 4),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62627, 62628, "shared-owner", "/srv/amp/owner-a", 1, 2),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62629, 62630, "shared-owner", "/srv/amp/owner-b", 3, 4),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62631, 62632, "path-a", "/srv/amp/shared", 1, 2),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62633, 62634, "path-b", "/srv/amp/shared", 3, 4),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62635, 62636, "inode-a", "/srv/amp/inode-a", 9, 9),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62637, 62638, "inode-b", "/srv/amp/inode-b", 9, 9),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62639, 62640, "anchor", "/srv/amp/anchor", 1, 2),
				)
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62643, 62644, "bound-gid", "/srv/amp/bound-gid", 1, 2),
				)
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62647, 62648, "stale-marker", "/srv/amp/stale-marker", 1, 2),
				)
...
func TestAgentStandaloneCovAuthorityRootAuditAcceptsACoherentRegistry(t *testing.T) {
	...
	first := agentStandaloneCovOwner(62651, 62652, "coherent-a", "/srv/amp/coherent-a", 11, 12)
	second := agentStandaloneCovOwner(62653, 62654, "coherent-b", "/srv/amp/coherent-b", 13, 14)
...
func TestAgentStandaloneCovOwnerUniquenessRefusesEveryCollision(t *testing.T) {
	want := agentStandaloneCovOwner(62501, 62502, "cov-uniqueness", "/srv/amp/cov-uniqueness", 71, 72)
	suffix := agentStandaloneCovSuffix
	for _, testCase := range []struct {
		name    string
		expired bool
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62501, 62502, "someone-else", "/srv/amp/someone-else", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62507, 62502, "gid-holder", "/srv/amp/gid-holder", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62509, 62510, "cov-uniqueness", "/srv/amp/other-root", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62511, 62512, "path-holder", "/srv/amp/cov-uniqueness", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62513, 62514, "inode-holder", "/srv/amp/other-inode-root", 71, 72),
				)
			},
...
func TestAgentStandaloneCovOwnerUniquenessAcceptsDisjointRegistryState(t *testing.T) {
	...
	agentStandaloneCovWriteOwner(t, directory,
		agentStandaloneCovOwner(62519, 62520, "disjoint", "/srv/amp/disjoint", 91, 92),
	)
...
func TestAgentStandaloneCovPriorDispositionRequiresTheExactRetainedMarker(t *testing.T) {
	ownerUID, ownerGID := agentStandaloneTestAuthorityIDs()
	owner := agentStandaloneCovOwner(62531, 62532, "prior-disposition", "/srv/amp/prior-disposition", 51, 52)
...
func TestAgentStandaloneCovOwnerClaimRefusesEveryUnsafeRegistryState(t *testing.T) {
	ownerUID, ownerGID := agentStandaloneTestAuthorityIDs()
	want := agentStandaloneCovOwner(62541, 62542, "cov-claim", "/srv/amp/cov-claim", 61, 62)

	t.Run("uid bound to another tuple", func(t *testing.T) {
		directory := openAgentStandaloneTestDirectory(t)
		agentStandaloneCovWriteOwner(t, directory,
			agentStandaloneCovOwner(62541, 62542, "another-claim", "/srv/amp/another-claim", 63, 64),
		)
	...
	t.Run("existing binding with a colliding peer", func(t *testing.T) {
		...
		agentStandaloneCovWriteOwner(t, directory, want)
		agentStandaloneCovWriteOwner(t, directory,
			agentStandaloneCovOwner(62543, 62542, "gid-collider", "/srv/amp/gid-collider", 65, 66),
		)
	...
	t.Run("colliding peer blocks a fresh binding", func(t *testing.T) {
		directory := openAgentStandaloneTestDirectory(t)
		agentStandaloneCovWriteOwner(t, directory,
			agentStandaloneCovOwner(62545, 62542, "fresh-collider", "/srv/amp/fresh-collider", 67, 68),
		)
