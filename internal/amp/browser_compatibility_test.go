...
func TestAuthLoginBrowserCompatibilityIsPlatformBound(t *testing.T) {
	...
	if err := checkAuthLoginBrowserCompatibility(darwinPlatform, "/amp", open); !errors.Is(err, errBrowserShimUnsupported) || !strings.Contains(err.Error(), want.Error()) {
		t.Fatalf("Darwin unreadable compatibility = %v", err)
	}

	open = func(string) (io.ReadCloser, error) {
		return io.NopCloser(strings.NewReader("prefix /usr/bin/open suffix")), nil
	}
	if err := checkAuthLoginBrowserCompatibility(darwinPlatform, "/amp", open); !errors.Is(err, errBrowserShimUnsupported) || !strings.Contains(err.Error(), "direct Darwin browser launcher") {
	...
	if err := checkAuthLoginBrowserCompatibility(darwinPlatform, "/amp", open); !errors.Is(err, errBrowserShimUnsupported) || !strings.Contains(err.Error(), "no audited headless account-login contract") {
	...
	open = func(string) (io.ReadCloser, error) {
		return io.NopCloser(strings.NewReader(linuxCompatibleAmpSource)), nil
	}
	...
	open = func(string) (io.ReadCloser, error) {
		return io.NopCloser(strings.NewReader(linuxCompatibleAmpSource + "/usr/bin/xdg-open")), nil
	}
	if err := checkAuthLoginBrowserCompatibility(linuxPlatform, "/amp", open); !errors.Is(err, errBrowserShimUnsupported) || !strings.Contains(err.Error(), "direct Linux browser launcher") {
		t.Fatalf("Linux direct-launch compatibility = %v", err)
	}

	open = func(string) (io.ReadCloser, error) { return io.NopCloser(strings.NewReader("unknown amp build")), nil }
	if err := checkAuthLoginBrowserCompatibility(linuxPlatform, "/amp", open); !errors.Is(err, errBrowserShimUnsupported) || !strings.Contains(err.Error(), "no audited PATH-mediated Linux") {
		t.Fatalf("Linux unknown compatibility = %v", err)
	}
}

const linuxCompatibleAmpSource = `async function openURL(T,R){try{let platform=currentPlatform(),options={timeout:5000,signal:R};switch(platform){case"win32":await spawn("rundll32",["url.dll,FileProtocolHandler",T],options);break;case"darwin":await spawn("open",[T],options);break;default:await spawn("xdg-open",[T],options);break}}}async function accountLogin(T){let url=makeURL(T);try{await openURL(url,abort.signal)}catch(error){log(error)}write("If your browser does not open automatically")}`

func TestAuthLoginBinaryInspectionIsStreaming(t *testing.T) {
	contents := "prefix" + darwinDirectBrowserLauncher + linuxCompatibleAmpSource + "suffix"
	inspection, err := inspectAuthLoginBinary(iotest.OneByteReader(strings.NewReader(contents)))
	if err != nil || !inspection.darwinDirectLauncher || !inspection.linuxAccountLoginUsesPath || inspection.digest != sha256.Sum256([]byte(contents)) {
...
func TestCheckAuthLoginBrowserCompatibilityUsesTheHostPlatform(t *testing.T) {
	...
	if runtime.GOOS == darwinPlatform || runtime.GOOS == linuxPlatform {
		if !errors.Is(err, errBrowserShimUnsupported) {
