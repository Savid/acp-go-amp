...
func TestKeystoreLinuxLoginNeverExecsABrowserLauncher(t *testing.T) {
	...
	probe := buildBrowserShimProbe(t, "integration")
...
func buildBrowserShimProbe(t *testing.T, tags string) string {
	t.Helper()

	out := filepath.Join(t.TempDir(), "browsershim.test")

	command := exec.Command("go", "test", "-c", "-tags="+tags, "-o", out, "./internal/amp")
	command.Dir = ".."
