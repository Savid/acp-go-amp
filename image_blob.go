...
// The block is weighed on its decoded length, the same quantity every image
// block is weighed on, so declaring bytes as an untyped blob buys a prompt no
// more of them than an image block gets and the advertised envelope stays a true
// statement of the tightest inbound byte gate.
//
