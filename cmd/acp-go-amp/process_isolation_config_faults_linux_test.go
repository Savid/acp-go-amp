...
func TestProcessIsolationValueValidatorsBoundTheirInputs(t *testing.T) {
	...
	// The last three are the authority root itself and anything beneath it,
	// plus the filesystem root. A state root that overlapped the durable
	// authority tree would let a claimed identity own the registry that
	// adjudicates its own claim.
	for _, value := range []string{
		"", "relative", "/var/lib/../lib", "/var/lib/acp\x00cov", "/var/lib/acp\ncov",
		"/" + strings.Repeat("a", 4096),
		"/", "/var/lib/acp-go/agent-identities", "/var/lib/acp-go/agent-identities/11.lock",
	} {
		if validProcessIsolationStateRoot(value) {
