...
func TestFakeAmpAgentHelper(t *testing.T) {
	...
	case "continue":
		...
		default:
			...
			os.Stdout.WriteString(`{"type":"system","subtype":"init","cwd":"/tmp/project","session_id":"T-agent-thread","tools":["Read"],"mcp_servers":[{"name":"svc","status":"connected"}],"agent_mode":"` + initMode + `"}` + "\n")
...
func fakeAgentAmpPath(t *testing.T, mode string) (string, string) {
	...
	// Fake-binary tests provide a key unless a test explicitly clears it.
