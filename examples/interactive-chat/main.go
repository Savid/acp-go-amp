...
func run(ctx context.Context, args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) int {
	...
	mode := flags.String("mode", "", "native Amp mode for the session")
	...
	go func() {
		served <- ampacp.Serve(serveCtx, agentReader, agentWriter, ampacp.WithLogger(slog.New(slog.DiscardHandler)))
	}()
	...
	err = chat(ctx, conn, input, cwd, *mode, stdout)
...
func chat(ctx context.Context, conn agentConnection, input *bufio.Reader, cwd string, mode string, stdout io.Writer) error {
	...
	if mode != "" {
		opts = append(opts, ampacp.WithSessionAmpOptions(ampacp.NewAmpOptions(ampacp.WithAmpMode(mode))))
	}
